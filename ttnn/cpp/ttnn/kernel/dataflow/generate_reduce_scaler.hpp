...
// Min/max with user `scalar=…`: `mul_tiles_bcast_scalar` reads a single B datum; the default sparse
// `generate_reduce_scaler` may not set the word the unpack/LLK uses, so the mul applies ×1.0 and output is ~0.5×
// the reference. Only used for c_2 tile 1 in min/max readers, not the unity tile 0 and not other ops.
template <bool half_tile = false>
FORCE_INLINE void generate_reduce_scaler_for_minmax_post_bcast(const uint32_t cb_id, const uint32_t scaler) {
    cb_reserve_back(cb_id, 1);
    constexpr uint32_t num_zeros_reads = (half_tile ? 1024 : 2048) / MEM_ZEROS_SIZE;
    static_assert(num_zeros_reads > 0, "num_zeros_reads must be greater than 0");
    uint64_t zeros_noc_addr = get_noc_addr(MEM_ZEROS_BASE);
