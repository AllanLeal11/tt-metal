...
IndexedFillProgramFactory::cached_program_t IndexedFillProgramFactory::create(
    ...
    uint32_t pages_per_batch = 0;
    uint32_t shard_page_size = 0;
    uint32_t shard_ppb = 0;
    uint32_t total_batches_per_core = 0;  // only used for shard_local

    // page_boundary: the first dimension index that belongs to the page unit itself.
    //   ROW_MAJOR: page = last dim row, so page_boundary = rank-1
    //   TILE:      page = tile spanning last 2 dims, so page_boundary = rank-2
    ...
    // outer_count = product of dims strictly before `dim` (1 for dim=0).
    // inner_count = product of page-unit counts for dims strictly between `dim` and
    //               page_boundary (includes tile dims for TILE layout).
    // For dim=0: outer_count=1, inner_count = all page-level counts = pages_per_batch.
    ...
    pages_per_batch = inner_count;  // for dim=0 this equals the old pages_per_batch

    const uint32_t S_dim = input_a.padded_shape()[dim];  // size of target dim
    const uint32_t outer_stride_a = S_dim * inner_count;
    const uint32_t outer_stride_b = b * inner_count;
    const uint32_t outer_stride_out = outer_stride_a;
    ...
    if (is_shard_local) {
        ...
        const uint32_t shard_width = shard_spec.shape[1];                            // elements
        const uint32_t H_N = input_a.padded_shape()[1] * input_a.padded_shape()[2];  // rows per batch

        if (is_tile) {
            shard_page_size = tt::tile_size(cb_data_format);
            const uint32_t shard_tile_h = H_N / tt::constants::TILE_HEIGHT;
            const uint32_t shard_tile_w = shard_width / tt::constants::TILE_WIDTH;
            shard_ppb = shard_tile_h * shard_tile_w;
        } else {
            shard_page_size = shard_width * input_a.element_size();
            shard_ppb = H_N;
        }
        ...
        if (input_a.memory_config().memory_layout() == TensorMemoryLayout::WIDTH_SHARDED) {
            ...
            // BLOCK_SHARDED: shard_height = B * H_N / n_y; batches_per_core = shard_height / H_N = B / n_y
            const uint32_t n_y = shard_spec.grid.bounding_box().grid_size().y;
            total_batches_per_core = B / n_y;
        }
    }
        ...
        if (is_native) {
            ...
            const uint32_t b_full_ppb_arg =
                is_tile ? (input_a.padded_shape()[1] * input_a.padded_shape()[2] / tt::constants::TILE_HEIGHT) *
                        ...
                        : input_a.padded_shape()[1] * input_a.padded_shape()[2];  // H_N
            const uint32_t shard_tile_w_arg = is_tile ? (shard_spec.shape[1] / tt::constants::TILE_WIDTH) : 1u;
            const uint32_t full_tile_w_arg = is_tile ? (input_a.padded_shape()[-1] / tt::constants::TILE_WIDTH) : 1u;
            const uint32_t col_page_offset_arg = is_tile ? cx * shard_tile_w_arg : 0u;
            const uint32_t col_byte_offset_arg = !is_tile ? cx * shard_spec.shape[1] * input_a.element_size() : 0u;
...
void IndexedFillProgramFactory::override_runtime_arguments(
    ...
    // Recompute outer/inner stride params (mirrors create()).
    uint32_t outer_count = 1;
    for (int64_t i = 0; i < dim; ++i) {
        outer_count *= input_a.padded_shape()[i];
    }
    ...
    const uint32_t outer_stride_b = b * inner_count;
    const uint32_t outer_stride_out = outer_stride_a;
    uint32_t batch_size_in_pages = inner_count;  // pages per (outer, slice) pair

    // For shard_local: recompute shard_ppb and per-core batch counts.
    uint32_t shard_ppb = 0;
