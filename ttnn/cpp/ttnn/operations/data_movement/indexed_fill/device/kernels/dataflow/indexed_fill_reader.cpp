...
void kernel_main() {
    ...
    if constexpr (IS_SHARD_LOCAL) {
        ...
        for (uint32_t b_local = 0; b_local < total_local_batches; ++b_local) {
            ...
            // Find whether this global batch is in the replacement set (last match wins).
            ...
            for (uint32_t p = 0; p < shard_ppb; ++p) {
                ...
                if (replace_b) {
                    if constexpr (IS_B_SAME_SHARDED) {
                        ...
                        //   where p_row = p / shard_tile_w, p_col = p % shard_tile_w.
                        const uint32_t p_row = p / shard_tile_w;
                        const uint32_t p_col = p % shard_tile_w;
