...
// offset baked in — so vararg indices in kernel code are stable across schema changes.
void write_kernel_args_generated_header(const string& out_dir, const JitBuildSettings& settings) {
    ...
    // Collect named args in declaration order (offsets are positional — do NOT sort).
    vector<string> rta_names;
    vector<string> crta_names;
    settings.process_named_runtime_args([&](const string& name) { rta_names.push_back(name); });
    settings.process_named_common_runtime_args([&](const string& name) { crta_names.push_back(name); });

    // Collect named CTAs. Sort by name for deterministic output (order does not affect semantics
    // since CTA values are embedded directly, but we want hash-friendly codegen).
    ...
    // Named RTAs: byte offset increments by 4 per arg (Phase 1 is uint32_t only).
    ...
    // Named CRTAs: separate offset space.
    ...
    // Named CTAs: emitted as CtaVal<uint32_t> alongside the RTA/CRTA accessors.
    ...
    // Vararg helpers. The offset (named_arg_count) is baked in so kernel code uses 0-based
