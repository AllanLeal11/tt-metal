...
#pragma GCC unroll 8
    for (int d = 0; d < ITERATIONS; d++) {
        ...
        TT_SFPSTORE(
            p_sfpu::LREG1, InstrModLoadStore::DEFAULT, ADDR_MOD_3, dst_index_out * SFP_DST_TILE_ROWS_64);  // store max

        // x = min(x, max_val) using LREG3
        TT_SFPLOAD(p_sfpu::LREG0, InstrModLoadStore::DEFAULT, ADDR_MOD_3, dst_index_out * SFP_DST_TILE_ROWS_64);
        TTI_SFPMOV(0, p_sfpu::LREG3, p_sfpu::LREG1, 0);
        TTI_SFPSWAP(0, p_sfpu::LREG1, p_sfpu::LREG0, 1 /* smaller value to LREG0 */);
        TT_SFPSTORE(
            p_sfpu::LREG0, InstrModLoadStore::DEFAULT, ADDR_MOD_3, dst_index_out * SFP_DST_TILE_ROWS_64);  // store min
