...
inline void calculate_comp_uint16(std::uint32_t dst_index_in, std::uint32_t dst_index_out) {
    ...
    for (int d = 0; d < ITERATIONS; d++) {
        ...
        TT_SFPLOAD(p_sfpu::LREG0, LO16, ADDR_MOD_3, dst_index_in * SFP_DST_TILE_ROWS_64);
        ...
        TTI_SFPENCC(0, 0, 0, 0);
        // store result
        TT_SFPSTORE(p_sfpu::LREG1, LO16, ADDR_MOD_3, dst_index_out * SFP_DST_TILE_ROWS_64);
        dst_reg++;
    }
}
...
inline void calculate_eqz_uint32(std::uint32_t dst_index_in, std::uint32_t dst_index_out) {
    ...
    for (int d = 0; d < ITERATIONS; d++) {
        TT_SFPLOAD(p_sfpu::LREG0, INT32, ADDR_MOD_3, dst_index_in * SFP_DST_TILE_ROWS_64);
        TTI_SFPLZ(0, 0, 1, 4);    // result in lreg1 is leading zero count
        TTI_SFPSHFT(0, 2, 1, 0);  // 32 >> 5 = 1 else 0
        TT_SFPSTORE(p_sfpu::LREG1, INT32, ADDR_MOD_3, dst_index_out * SFP_DST_TILE_ROWS_64);
        dst_reg++;
    }
}
